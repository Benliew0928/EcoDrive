...
const styleSheet = `
  ...
  .city3d-world { aspect-ratio:1; left:50%; position:absolute; top:49%; transform:translate(-50%,-50%) rotateX(var(--terrain-pitch)) rotateZ(var(--terrain-yaw)) scale(var(--zoom-scale, 1)); transform-origin:center; transform-style:preserve-3d; transition:transform .09s linear; width:min(90%,920px); }
  ...
  .city3d-overview-actions{display:grid;gap:16px;grid-template-columns:repeat(2,1fr);min-height:0}.city3d-launch-card{color:#effff7;cursor:pointer;display:flex;flex-direction:column;justify-content:center;min-height:0;padding:36px 40px;text-align:left;transition:.2s ease}.city3d-launch-card:hover{border-color:rgba(55,229,143,.55);transform:translateY(-2px)}.city3d-launch-card--city{background:radial-gradient(circle at 85% 20%,rgba(56,189,248,.16),transparent 40%),linear-gradient(145deg,#0d2424,#081514)}.city3d-launch-card--store{background:radial-gradient(circle at 85% 20%,rgba(245,184,75,.16),transparent 40%),linear-gradient(145deg,#211c10,#0e1512)}.city3d-launch-card>span{color:#749087;font-size:16px;font-weight:900;text-transform:uppercase}.city3d-launch-card strong{font-size:40px;margin:10px 0;font-weight:950}.city3d-launch-card small{color:#b3cbc1;font-size:20px;line-height:1.55}.city3d-launch-card b{color:#37e58f;font-size:22px;margin-top:auto;padding-top:24px}.city3d-launch-card--store b{color:#f5b84b}
