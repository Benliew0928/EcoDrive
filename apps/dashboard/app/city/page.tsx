...
export default function CityPage() {
  ...
  const terrainDragRef = useRef({ active: false, startX: 0, startY: 0, startYaw: 0, startPitch: 56, latestYaw: -4, latestPitch: 56 });
  ...
  const beginTerrainRotation = (event: PointerEvent<HTMLDivElement>) => {
    if (storeOpen) return;
    terrainDragRef.current = { active: true, startX: event.clientX, startY: event.clientY, startYaw: terrainYaw, startPitch: terrainPitch, latestYaw: terrainYaw, latestPitch: terrainPitch };
  ...
  const updateTerrainPointer = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const newYaw = clampView(terrainDragRef.current.startYaw + (event.clientX - terrainDragRef.current.startX) * sensitivity, -45, 45);
    const newPitch = clampView(terrainDragRef.current.startPitch - (event.clientY - terrainDragRef.current.startY) * sensitivity, 30, 80);
    
    terrainDragRef.current.latestYaw = newYaw;
    terrainDragRef.current.latestPitch = newPitch;
    
    if (terrainRef.current) {
        terrainRef.current.style.setProperty('--terrain-yaw', `${newYaw}deg`);
        terrainRef.current.style.setProperty('--terrain-pitch', `${newPitch}deg`);
        terrainRef.current.style.setProperty('--terrain-yaw-inverse', `${-newYaw}deg`);
        terrainRef.current.style.setProperty('--terrain-pitch-inverse', `${-newPitch}deg`);
    }
  };
  ...
  const stopTerrainRotation = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (terrainDragRef.current.latestYaw !== undefined) {
      setTerrainYaw(terrainDragRef.current.latestYaw);
      setTerrainPitch(terrainDragRef.current.latestPitch);
    }
    if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);
  };

  const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    if (cityView === "store") return;
    setZoomScale((prev) => {
      const factor = 0.08;
      const nextZoom = event.deltaY < 0 ? prev + factor : prev - factor;
      return Math.max(0.4, Math.min(2.2, nextZoom));
    });
  };
        ...
        <section className="city3d-frame" aria-labelledby="city3d-title">
          <header className="city3d-header">
            <div>
              ...
              {cityView !== "overview" && <span>{cityView === "store" ? "Choose an asset, then place it precisely on your terrain." : "Explore, edit and grow your sustainable district."}</span>}
            </div>
            ...
            <section className="city3d-overview" aria-label="City command center overview">
              ...
              <div className="city3d-overview-side">
                <div className="city3d-overview-metrics">
                  ...
                  <article><span>Total Yield Coins</span><strong className="is-green">{cityStats.yieldCoins.toLocaleString()}</strong></article>
                  <article><span>Passive income</span><strong className="is-cyan">+{cityStats.dailyYield}<em>/ day</em></strong></article>
                </div>
          ...
          <div className={storeOpen ? "city3d-workspace city3d-workspace--store" : "city3d-workspace"}>
            <section className="city3d-map-panel" aria-label="Interactive 3D eco-city terrain">
              ...
              >
                ...
                <div className="city3d-world" style={{ "--zoom-scale": zoomScale } as CSSProperties}>
...
const styleSheet = `
  ...
  .city3d-map-heading > div:first-child { display: grid; gap: 3px; }.city3d-map-heading small,.city3d-palette-heading small { color: #37e58f; font-size: 14px; font-weight: 900; letter-spacing: .1em; text-transform: uppercase; }.city3d-map-heading strong { color: #f4fff9; font-size: 20px; }
  .city3d-message { background: rgba(55,229,143,.07); border: 1px solid rgba(55,229,143,.2); border-radius: 8px; color: #8feeb9; font-size: 14px; line-height: 1.35; max-width: 480px; padding: 10px 14px; }.city3d-message--error { background: rgba(255,95,80,.08); border-color: rgba(255,95,80,.28); color: #ff867a; }
  ...
  .city3d-world { aspect-ratio:1; left:50%; position:absolute; top:49%; transform:translate(-50%,-50%) rotateX(var(--terrain-pitch)) rotateZ(var(--terrain-yaw)) scale(var(--zoom-scale, 1)); transform-origin:center; transform-style:preserve-3d; transition:transform 0.4s cubic-bezier(0.2, 0.8, 0.2, 1); width:min(90%,920px); }
  .city3d-field.is-dragging .city3d-world { transition:none; }
  ...
  .city3d-asset-label{background:rgba(4,13,12,.88);border:1px solid rgba(178,218,205,.2);border-radius:999px;bottom:-1px;color:#eafff5;font-size:8px;font-weight:900;left:50%;padding:4px 8px;position:absolute;transform:translateX(-50%) translateZ(48px) rotateX(90deg);white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .15s ease}.city3d-asset:hover .city3d-asset-label,.city3d-asset--selected .city3d-asset-label{opacity:1}
  ...
  /* LowPoly structure */
  .lowpoly { bottom:0;display:block;height:88px;left:10px;position:absolute;transform:scale(0.98);transform-origin:center bottom;transform-style:preserve-3d;width:88px}
  ...
  .city3d-instruction{background:rgba(4,13,13,.78);border:1px solid rgba(142,165,160,.2);border-radius:999px;bottom:12px;color:#81968f;font-size:16px;left:12px;padding:12px 18px;pointer-events:none;position:absolute;z-index:160}.city3d-legend{display:none}
  ...
  .city3d-store-toggle { min-height: 48px; font-size: 14px; padding: 0 18px; }
  ...
  .city3d-world { width: min(85%, 820px); }
  ...
  .city3d-view-toggle{background:rgba(139,164,157,.08);border:1px solid rgba(139,164,157,.28);border-radius:8px;color:#dcece7;cursor:pointer;font-size:14px;font-weight:900;min-height:48px;padding:0 18px;white-space:nowrap}.city3d-view-toggle--active{background:rgba(55,229,143,.12);border-color:rgba(55,229,143,.42);color:#37e58f}
  ...
  .city3d-builder-kpis{align-items:stretch;display:flex;flex:1;gap:8px;min-width:0}
  .city3d-builder-kpis>span{background:rgba(10,24,22,.78);border:1px solid rgba(91,121,113,.22);border-radius:9px;display:flex;flex:1;flex-direction:column;justify-content:center;min-width:125px;padding:8px 12px}
  .city3d-builder-kpis small{color:#708a82;font-size:11px;font-weight:900;letter-spacing:.05em;text-transform:uppercase}
  .city3d-builder-kpis strong{align-items:center;color:#eefdf6;display:flex;font-size:18px;gap:6px;line-height:1.2}.city3d-builder-kpis strong.is-cyan{color:#38bdf8}
  .city3d-back-button{font-size:14px;min-width:96px}
